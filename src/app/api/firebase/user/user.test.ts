...
import { GET, POST } from "@/app/api/firebase/user/route"
...
describe("firebase/user", () => {
	...
	it("should fail when email input are not acceptable", async () => {
		...
		expect(await response.json()).toStrictEqual({
			error: "Email must be a string matching /^[\\w-\\.]+@([\\w-]+\\.)+[\\w-]{2,4}$/.",
		})
	})
	...
	it("should fail when password input are not acceptable", async () => {
		...
		expect(response.status).toBe(400)
		expect(await response.json()).toStrictEqual({
			error: "Password must be (is not {} and a string matching /^[a-z|A-Z|0-9|!|\\$|@|?|#|%|\\^]+$/ and a collection or string with size a number greater than <5>).",
		})
	})
