...
describe("Checklist", () => {
	describe("CronJobCheckList", () => {
		...
		it("should render as inactive if post is not found", () => {
			...
			expect(screen.getByText("False")).toBeInTheDocument()
			expect(
				screen.getByText("Cron execution pending"),
			).toBeInTheDocument()
		})
		...
		it("should render as active if post is there", async () => {
			...
			expect(screen.getByText("CronJob")).toBeInTheDocument()
			expect(
				within(screen.getByTestId("result Since Deployment")).getByText(
					"True",
				),
			).toBeInTheDocument()
			expect(
				await within(
					screen.getByTestId("message Since Deployment"),
				).findByText("12/12/2024"),
			).toBeInTheDocument()
			expect(
				await within(
					screen.getByTestId("message Today's Run"),
				).findByText("12/12/2024"),
			).toBeInTheDocument()
