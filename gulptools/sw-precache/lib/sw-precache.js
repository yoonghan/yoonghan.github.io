...
function escapeHtml(relativeUrl) {
  var escapeKeyword = '.html',
      fixedKeyword = '/index.html',
      len = relativeUrl.length;
  var split = relativeUrl.substr(len - escapeKeyword.length, len),
      splitFixed = relativeUrl.substr(len - fixedKeyword.length, len);
  if( split === escapeKeyword && splitFixed !== fixedKeyword) {
    return relativeUrl.substr(0, relativeUrl.length - escapeKeyword.length);
  }
  return relativeUrl;
}
    ...
    params.staticFileGlobs.forEach(function(globPattern) {
      ...
      filesAndSizesAndHashes.forEach(function(fileAndSizeAndHash) {
        if (fileAndSizeAndHash.size <= params.maximumFileSizeToCacheInBytes) {
          ...
          relativeUrl = escapeHtml(relativeUrl);
