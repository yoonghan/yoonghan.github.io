define(["require", "exports"], function (require, exports) {
    var Footer = (function () {
        function Footer(tweenMax, timelineMax) {
        ...
        Footer.prototype.duplicateFooter = function () {
        ...
        Footer.prototype.addClick = function (footer, dupFooter) {
            var footerClickOpen = footer.getElementsByClassName('footer-pointer')[0], footerClickClose = dupFooter.getElementsByClassName('footer-pointer')[0], that = this;
            footer.addEventListener('click', function (event) {
                that.execOpen(that);
            });
            dupFooter.addEventListener('click', function (event) {
                that.execClose(that);
            });
        };
        Footer.prototype.execOpen = function (that) {
            var dupFooter = that.dupFooter, pointer = dupFooter.getElementsByClassName('footer-pointer')[0], tweenMax = that.tweenMax;
            dupFooter.style.display = 'block';
            tweenMax.to(pointer, 0.3, { rotation: '180' });
            tweenMax.to(dupFooter, 0.3, { height: '100vh' });
        };
        Footer.prototype.execClose = function (that) {
            var dupFooter = that.dupFooter, pointer = dupFooter.getElementsByClassName('footer-pointer')[0], tweenMax = that.tweenMax;
